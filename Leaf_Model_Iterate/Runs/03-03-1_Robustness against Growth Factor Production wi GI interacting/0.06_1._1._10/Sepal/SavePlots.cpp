...
for (int i=0;i<nbOutput;i++){
  ...
  } else if(i==10) {  //Growth factor 
    if (savePicScaled){
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthRate);
    } else{
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthFac, bb=savePicBounds);
    }
  } else if(i==11) {  //Growth rate
    if (savePicScaled){
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthRate);
    } else{
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthRate, bb=savePicBounds);
    }
  } else if(i==12) {  //Growth inhibitor concentration
    if (savePicScaled){
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthInh);
    } else{
      plot(listOutput[i],ps=f,fill=1,grey=1,value=1, viso=visoGrowthInh, bb=savePicBounds);
    }
  } else{             // Others
    if (savePicScaled){
