...
func bool EndSim(bool ROSFront, real ROSheight, real areacurr, real MaxArea, real ygain){
	bool EndSimul = 0;
	if (ROSFront){
		if (ROSheight < 0.){
			EndSimul=1;
		}
	} else if (areacurr > MaxArea) {
		EndSimul=1;
	} else if (ygain < minYgain){
		EndSimul=1;
	}
	return EndSimul;
}
...
// Response function to growth inhibitor
func real prefb(real DensInh, real RelElFactorInh, real RhzInh, real dRhoInh, int prefbCurve){

	// Prefb is a unitless, monotonously non-decreasing function of DensInh from 0 to 1
	real Prefb;
	
	//>> Tanh function, note that -1 is not reached
	if (prefbCurve == 1){
		Prefb = tanh((DensInh-RhzInh)/dRhoInh)/2+.5;
	}

	//>> Linear function with plateau
	else if (prefbCurve == 2){
		Prefb = min(1.0, DensInh/RhzInh);
	}

	//>> Step function
	else if (prefbCurve == 3){
		if (DensInh >= RhzInh){
			Prefb = 1.0;
		}
		else Prefb = 0.0;
	...
	//>> Hill's function, default. If dRho > 10, use step function instead.
		...
		Prefb = pow(DensInh,dRhoInh) / (pow(DensInh,dRhoInh)+pow(RhzInh,dRhoInh));
	}

	// The return value is a multiplicative factor on elasticity
	return exp(RelElFactorInh*Prefb);
}



// Response function to growth factor AND the growth inhibitor (they interact)
func real prefab(real Dens, real DensInh, real RelElFactor, real Rhz, real dRho, int prefaCurve){
	...
	// If the growth inhibitor concentration is greater than a value, growth stops
	if (DensInh > RhzInh){
		Prefa = -2.;
	}
...
// Get the production of growth inhibitor 2 (from the top) based on growth factor concentration
func real getGI2fromGF(real GFconc){

	real GI2prod;

	//>> Step function
	//	if (GFconc >= GI2transitionalGFconc){
	//		GI2prod = 0.;
	//	}
	//	else GI2prod = GI2maxProd;

	//>> Hill's function
	GI2prod = GI2maxProd * (1. - pow(GFconc,GI2power) / (pow(GFconc,GI2power)+pow(GI2transitionalGFconc,GI2power)));
	return GI2prod;
}
...
// Get Anisotropy based on height
func real getAnis(real height){
	//return max(AnisEnd, AnisStart + (AnisEnd-AnisStart) / (2.*frontArrHeightIni) * height);
	return max(AnisEnd, AnisStart + (AnisEnd-AnisStart) / 12. * height);
}
