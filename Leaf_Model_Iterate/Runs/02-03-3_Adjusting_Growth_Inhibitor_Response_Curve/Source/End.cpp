...
problem densityInh(RhoInh,wInh)=
	...
	+ int2d(sepal)(
		RhoInh*(dx(ux)+dx(uy))*wInh  // Ne marche pas?
	)
//    + int1d(sepal,1)(0*w)
	- int1d(sepal,4)(
		JInh*wInh
...
while (step<maxstep && !(EndSimul)){
	...
	for (int i=0;i<sepal.nv;i++){
		...
		ElastVertices(i) = prefa(CurrDensityV, RelElFactor, Rhz, dRho, prefaCurve)*prefb(DensityInhI, RelElFactorInh, RhzInh, dRhoInh, prefbCurve)*GetElastVert(i, ElastMean, ElastSd, CurrElastMean, CurrElastSd, MinElast, CurrElastV, ElastCoefTimeVar, sepal);
