...
for (int i=0;i<nbvertices;i++){
	ElastVertices[i] = prefa(DensityI, DifRelEl, Rhz, dRho)*GetElastFromMecaParam(ElastMean, ElastSd, MinElast);
...
while (step<maxstep && !(EndSimul)){
	...
	for (int i=0;i<sepal.nv;i++){
		...
		ElastVertices(i) = prefa(CurrDensityV, DifRelEl, Rhz, dRho)*GetElastVert(i, ElastMean, ElastSd, CurrElastMean, CurrElastSd, MinElast, CurrElastV, ElastCoefTimeVar, sepal);
